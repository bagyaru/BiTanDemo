...
//    paraStyle.lineSpacing = attributedString.lineSpacing;
//    paraStyle.hyphenationFactor = 1.0;
//    paraStyle.firstLineHeadIndent = 0.0;
//    paraStyle.paragraphSpacingBefore = 0.0;
//    paraStyle.headIndent = 0;
//    paraStyle.tailIndent = 0;
//    NSDictionary *dic = @{NSFontAttributeName:attributedString.font, NSParagraphStyleAttributeName:paraStyle, NSKernAttributeName:@(0.0)
//                          };
//    CGSize size = [[getUserCenter DetermineWhetherTheFuturesOrCurrencyOnThePost:attributedString.string] boundingRectWithSize:CGSizeMake(maxWidth, 1000000) options:NSStringDrawingUsesLineFragmentOrigin attributes:dic context:nil].size;
        ...
        [lines enumerateObjectsUsingBlock:^(id  _Nonnull line, NSUInteger idx, BOOL * _Nonnull stop) {
            if (idx <= lineNum - 1) {
                ...
                if (idx == lineNum - 1) {
                    ...
                    handlingLineAttrStr = [lineAttrStr attributedSubstringFromRange:NSMakeRange(0, lineAttrStr.length > 5 ? (lineAttrStr.length - deleteStringLength - 3) : (lineAttrStr.length > 2 ? lineAttrStr.length - deleteStringLength : 0))];
