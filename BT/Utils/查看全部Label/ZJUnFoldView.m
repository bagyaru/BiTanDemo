                               ...
                               location:(UnFoldButtonLocation)location
{
    ...
    if (_isMutilLine) { // 4.1 有多行
        ...
        if (_isSurpassFoldLine)
        {
            ...
            //bugfix
            _foldLabelH = foldLines == 4 ? (_foldLabelH > 100 ? 100 : _foldLabelH) : (_foldLabelH > 150 ? 150 : _foldLabelH);
            NSLog(@"收起：%@=======%.0f======%.0f",contentAttributedString.string,_foldLabelH,CGRectGetMaxY(foldLineRect));
            ...
            //bugfix
            //_unFoldLabelH = maxHeight;
            NSLog(@"展开：%@=======%.0f",contentAttributedString.string,_unFoldLabelH);
            // 4.1.2.4 宽度
...
- (void)unFoldAction
{
    ...
    if (_unFoldActionBlock) {
        _unFoldActionBlock(_isAddOneLine);
        NSLog(@"展开高度=====%.0f 折叠高度=====%.0f",_unFoldLabelH,_foldLabelH);
    }
...
- (void)updateUnFoldViewFrame
{
    ...
    if (_isUnFold) {
        ...
        //selfH = _isAddOneLine ? _unFoldLabelH +24 : _unFoldLabelH;
    } else {
