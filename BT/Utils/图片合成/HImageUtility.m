...
+ (UIImage *)composeImageOnMainImage:(UIImage *)mainImage
    ...
    for (UIImage *img in imgArray) {
        ...
        CGRect fristRect = CGRectFromString(string);
        NSLog(@"=======%.0f=======%.0f",fristRect.size.width,fristRect.size.height);
...
+ (UIImage *)imageWithText:(NSString *)text
    ...
    NSString *mark = text;
    CGFloat height = [mark sizeWithPreferWidth:textFrame.size.width font:[UIFont fontWithName:@"STHeitiSC-Light" size:fontSize]].height; // 此分类方法要导入头文件
...
//    UIGraphicsBeginImageContext(viewFrame.size);
    UIGraphicsBeginImageContextWithOptions(viewFrame.size, NO, 2.0);
    [image drawInRect:CGRectMake(0, 0, viewFrame.size.width, viewFrame.size.height)];
    NSDictionary *attr = @{NSFontAttributeName: [UIFont fontWithName:@"STHeitiSC-Light" size:fontSize], NSForegroundColorAttributeName : textColor };
    //位置显示
...
+ (UIImage *)imageWithText:(NSString *)text title:(NSString*)title
    ...
    [wordStyle setLineSpacing:6];
    wordStyle.lineBreakMode = NSLineBreakByWordWrapping;
    NSDictionary *attr = @{NSFontAttributeName: [UIFont fontWithName:@"STHeitiSC-Light" size:fontSize], NSForegroundColorAttributeName : textColor,NSParagraphStyleAttributeName:wordStyle};
    CGFloat height = [mark sizeWithPreferWidth:textFrame.size.width attribute:attr].height;
    //位置显示
    
    UIImage *iconImage =[UIImage imageNamed:@"icon_time"];
    [iconImage drawInRect:CGRectMake(20, textFrame.origin.y, 18, 18)];
    
    //CGFloat titleHeight = [title sizeWithPreferWidth:textFrame.size.width font:[UIFont fontWithName:@"PingFangSC-Regular" size:22]].height; // 此分类方法要导入头文件
    [SAFESTRING(dateStr) drawInRect:CGRectMake(18+20+10, textFrame.origin.y-2, textFrame.size.width-60, 20) withAttributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFangSC-Regular" size:14], NSForegroundColorAttributeName : kHEXCOLOR(0xAAAAAA)}];
    //[title drawInRect:CGRectMake(20, textFrame.origin.y+20+15, textFrame.size.width, titleHeight) withAttributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFangSC-Regular" size:22], NSForegroundColorAttributeName : kHEXCOLOR(0x333333)}];
    
    CGFloat topHeight;
    if(title.length ==0){
        topHeight =0;
    }else{
        // topHeight = titleHeight + 15;
    }
    [mark drawInRect:CGRectMake(textFrame.origin.x, textFrame.origin.y+15+20, textFrame.size.width, height) withAttributes:attr];
...
//
+ (UIImage*)imageWithText:(NSString *)text font:(UIFont *)font textColor:(UIColor *)textColor image:(UIImage *)image imageFrame:(CGRect)imageFrame textFrame:(CGRect)textFrame{
    ...
    UIGraphicsBeginImageContextWithOptions(imageFrame.size, NO, 2.0);
    [image drawInRect:CGRectMake(0, 0, imageFrame.size.width, imageFrame.size.height)];
    NSDictionary *attr = @{NSFontAttributeName: font, NSForegroundColorAttributeName : textColor,NSParagraphStyleAttributeName:paragraphStyle};
    //位置显示
    [mark drawInRect:CGRectMake(textFrame.origin.x, textFrame.origin.y, textFrame.size.width, textFrame.size.height) withAttributes:attr];
