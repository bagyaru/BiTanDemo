...
+ (NSString *)signatureWithRequeseMethod:(NSString *)requestMethod
    ...
    //根据火币 的code 得到存储的 apiSecret
    NSString *apiSecret = [AppHelper apiSecretWithExchangeCode:@"huobi.pro"];//kHBSecretKey; //
    signature = [SYYEncryptionUtils hmac:signatureOriginalStr withKey:SAFESTRING(apiSecret)];
...
+ (NSMutableDictionary *)requiredParams{
    //根据火币 的code 得到存储的 apikey
    NSString *apiKey = [AppHelper apikeyWithExchangeCode:@"huobi.pro"];//kHBAccessKey ;//
