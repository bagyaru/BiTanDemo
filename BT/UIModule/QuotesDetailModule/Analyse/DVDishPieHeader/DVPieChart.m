...
#define COLOR_ARRAY @[\
[UIColor colorWithHexString:@"108ee9" andAlpha:1.0],\
[UIColor colorWithHexString:@"108ee9" andAlpha:0.8],\
[UIColor colorWithHexString:@"108ee9" andAlpha:0.6],\
[UIColor colorWithHexString:@"108ee9" andAlpha:0.4],\
[UIColor colorWithHexString:@"108ee9" andAlpha:0.2],\
[UIColor colorWithHexString:@"108ee9" andAlpha:0.05]\
]
...
//    CGFloat min = self.bounds.size.width > self.bounds.size.height ? self.bounds.size.height : self.bounds.size.width;
    ...
    CGPoint center =  CGPointMake(self.bounds.size.width * 0.5, self.bounds.size.height * 0.5 +6);
    CGFloat radius = 140 / 2; //
    //    CGFloat radius = min * 0.5 - CHART_MARGIN;
    CGFloat start = M_PI *5 / 4;
    CGFloat angle = 0;
    CGFloat end = start;
    ...
    if (self.dataArray.count == 0) {
        ...
        for (int i = 0; i < self.dataArray.count; i++) {
            ...
            // 获取弧度的中心角度
            CGFloat radianCenter = (start + end) * 0.5 - 5/4 *M_PI;
            
            
            // 获取指引线的起点
            CGFloat lineStartX = self.frame.size.width * 0.5 + radius * -cos(radianCenter);
            CGFloat lineStartY = self.frame.size.height * 0.5 + radius * -sin(radianCenter) +6;
    ...
    centerView.frame = CGRectMake(0, 0, 90, 90);
...
- (void)drawLineWithPointArray:(NSArray *)pointArray centerArray:(NSArray *)centerArray {
    ...
    for (int i = 0; i < pointArray.count; i++) {
        ...
        // 颜色（绘制数据时要用）,都用第一种 蓝色
        UIColor *color = [UIColor colorWithHexString:@"108ee9" andAlpha:1.0];
        ...
        if(model.rate*100 <0.01){
            number = [NSString stringWithFormat:@"<0.01%%"];
        }
        ...
        // 指引线转折点的位置(x, y)
        CGFloat breakPointX = x + 15 * -cos(radianCenter);
        CGFloat breakPointY = y + 15 * -sin(radianCenter);
        
        // 转折点到中心竖线的垂直长度（为什么+20, 在实际做出的效果中，有的转折线很丑，+20为了美化）
        CGFloat margin = fabs(width - breakPointX) + 15;
        ...
        rect.origin = CGPointMake(startX + movePoint, startY - 3);
        view.frame = rect;
        view.layer.cornerRadius = 3;
        view.layer.masksToBounds = true;
        
        //指引线上面的数字
        [name drawInRect:CGRectMake(numberX, numberY, numberWidth, numberHeight) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12.0], NSForegroundColorAttributeName:color,NSParagraphStyleAttributeName:paragraph}];
        
        // 指引线下面的title
        [number drawInRect:CGRectMake(titleX, titleY, titleWidth, titleHeight) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12.0],NSForegroundColorAttributeName:color,NSParagraphStyleAttributeName:paragraph}];
