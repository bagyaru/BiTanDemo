...
- (void)setCurrentDataMaxAndMin{
    ...
    if (self.dataSet.data.count > 0) {
        ...
        for (NSInteger i = idx; i < self.startDrawIndex + self.countOfshowCandle && i < self.dataSet.data.count; i++) {
            ...
            if (priEntity) {
                if (entity.high > self.maxHigh) {
                    self.maxHigh = entity.high;
                }
                if (entity.low < self.minLow) {
                    self.minLow = entity.low;
                }
            }else{
                self.maxHigh = entity.high;
                self.minLow = entity.low;
          ...
          isDrawRight:(BOOL)isDrawRight
{
    ...
    NSDictionary * drawAttributes = self.avgLabelAttributedDic ?:self.maAttributeDic;
...
//
//    //////ma5 ma10
    startP.y = startP.y -13+(radius/2.0)+2;

    CGFloat labelY =self.contentTop -13+(radius/4.0);
    //Background
...
//    }
    ...
    self.candleCoordsScale = (self.uperChartHeightScale * (self.contentHeight - self.xAxisHeitht))/(self.maxPrice-self.minPrice);
    ...
    for (NSInteger i = idex ; i< self.dataSet.data.count; i ++) {
        ...
        YKLineEntity  * entity  = [self.dataSet.data objectAtIndex:i];
        CGFloat open = ((self.maxPrice - entity.open) * self.candleCoordsScale) + top;
        CGFloat close = ((self.maxPrice - entity.close) * self.candleCoordsScale) + top;
        CGFloat high = ((self.maxPrice - entity.high) * self.candleCoordsScale) + top;
        CGFloat low = ((self.maxPrice - entity.low) * self.candleCoordsScale) + top;
        ...
        //date
        //日期 时间 //if (i > self.startDrawIndex+5 && i < self.dataSet.data.count - 2)
         //if (i % (NSInteger)(180/self.candleWidth) == 0)
        if (i > self.startDrawIndex && i < self.dataSet.data.count) {
            if(self.candleWidth<5){
                if (i % 30 == 0) {
                    ...
                    //时间
                    [self drawLabel:context attributesText:dateStrAtt rect:CGRectMake(startX - dateStrAttSize.width/2,((self.uperChartHeightScale * self.contentHeight)+ self.contentTop)+3, dateStrAttSize.width, dateStrAttSize.height)];
        ...
        if (entity.high == self.maxHigh) {
            if (!isMaxHeight) {
                NSString * date = [NSString stringWithFormat:@"←%@",entity.highStr];
                NSDictionary * drawAttributes = self.xAxisAttributedDic?:self.defaultAttributedDic;
                NSMutableAttributedString * dateStrAtt = [[NSMutableAttributedString alloc]initWithString:date attributes:drawAttributes];
                CGSize dateStrAttSize = [dateStrAtt size];
                if (startX + dateStrAttSize.width > self.contentRect.origin.x + self.contentRect.size.width) {
                    NSString * date = [NSString stringWithFormat:@"%@→",entity.highStr];
                    NSDictionary * drawAttributes = self.xAxisAttributedDic?:self.defaultAttributedDic;
                    NSMutableAttributedString * dateStrAtt = [[NSMutableAttributedString alloc]initWithString:date attributes:drawAttributes];
                    [self drawLabel:context attributesText:dateStrAtt rect:CGRectMake(startX - dateStrAttSize.width,high - dateStrAttSize.height / 2.0, dateStrAttSize.width, dateStrAttSize.height)];
                }else{
                    [self drawLabel:context attributesText:dateStrAtt rect:CGRectMake(startX,high - dateStrAttSize.height / 2.0, dateStrAttSize.width, dateStrAttSize.height)];
                }
                isMaxHeight = YES;
            }
        ...
        if (entity.low == self.minLow) {
            if (!isMinLow) {
                NSString * date = [NSString stringWithFormat:@"←%@",entity.lowStr];
                NSDictionary * drawAttributes = self.xAxisAttributedDic?:self.defaultAttributedDic;
                NSMutableAttributedString * dateStrAtt = [[NSMutableAttributedString alloc]initWithString:date attributes:drawAttributes];
                CGSize dateStrAttSize = [dateStrAtt size];
                if (startX + dateStrAttSize.width > self.contentRect.origin.x + self.contentRect.size.width) {
                    date = [NSString stringWithFormat:@"%@→",entity.lowStr];
                    NSDictionary * drawAttributes = self.xAxisAttributedDic?:self.defaultAttributedDic;
                    NSMutableAttributedString * dateStrAtt = [[NSMutableAttributedString alloc]initWithString:date attributes:drawAttributes];
                    [self drawLabel:context attributesText:dateStrAtt rect:CGRectMake(startX - dateStrAttSize.width,low - dateStrAttSize.height / 2.0, dateStrAttSize.width, dateStrAttSize.height)];
                }else{
                    [self drawLabel:context attributesText:dateStrAtt rect:CGRectMake(startX,low - dateStrAttSize.height / 2.0, dateStrAttSize.width, dateStrAttSize.height)];
                }
                 isMinLow = YES;
            }
    ...
    for (NSInteger i = idex ; i< self.dataSet.data.count; i ++) {
        ...
        if (self.highlightLineCurrentEnabled) {
            if (i == self.highlightLineCurrentIndex) {
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:NSNotification_ShowWarningRotationView object:entity userInfo:nil];
                ...
                ////MA5 MA10 MA20
                BOOL isDrawRight = NO;  //startX < (self.contentRight)/2.0;
...
- (void)handlePanGestureAction:(UIPanGestureRecognizer *)recognizer
{
    ...
    [self handkeGesture:recognizer];

}

- (void)handkeGesture:(UIPanGestureRecognizer *)recognizer{
