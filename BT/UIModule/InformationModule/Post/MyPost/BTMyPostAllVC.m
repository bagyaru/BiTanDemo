...
@interface BTMyPostAllVC ()<UITableViewDelegate,UITableViewDataSource,BTLoadingViewDelegate>
...
- (void)requestList:(RefreshState)state{
    ...
    BTMyPostRequest *api = [[BTMyPostRequest alloc] initWithCurrentPage:self.pageIndex original:self.original];
    [api requestWithCompletionBlockWithSuccess:^(__kindof BTBaseRequest *request) {
        ...
        if (state == RefreshStatePull || state == RefreshStateNormal) {
            ...
            for (NSDictionary *dic in request.data) {
                ...
                if (!self.original) {
                    model.whereVC = @"我的帖子-全部";
                }else {
                    model.whereVC = @"我的帖子-原创";
                }
