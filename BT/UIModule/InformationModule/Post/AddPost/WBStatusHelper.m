...
+ (NSString *)stringWithTimeInterval:(NSString *)time{
    ...
    dispatch_once(&onceToken, ^{
        formatterYesterday = [[NSDateFormatter alloc] init];
        [formatterYesterday setDateFormat:[NSString stringWithFormat:@"%@ HH:mm",[APPLanguageService wyhSearchContentWith:@"zuotian"]]];
        [formatterYesterday setLocale:[NSLocale currentLocale]];
    ...
    if (delta < -60 * 10) { // 本地时间有问题
        ...
        return [NSString stringWithFormat:@"%d%@", (int)(delta / 60.0),[APPLanguageService wyhSearchContentWith:@"fengzhongqian"]];
    } else if (date.isToday) {
        return [NSString stringWithFormat:@"%d%@", (int)(delta / 60.0 / 60.0),[APPLanguageService wyhSearchContentWith:@"xiaoshiqian"]];
    } else if (date.isYesterday) {
        return [formatterYesterday stringFromDate:date];
    } else if (date.year == now.year) {
        return [formatterSameYear stringFromDate:date];
