...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = isNightMode?ViewContentBgColor :CWhiteColor;
...
- (void)setUpProperties
{
    ...
    [preBtn setTitleColor:MainBg_Color forState:UIControlStateNormal];
...
- (void)setShowsCancelButton:(BOOL)showsCancelButton{
    ...
    if (showsCancelButton) {
        ...
        [cancelBtn setTitleColor:MainBg_Color forState:UIControlStateNormal];
        [cancelBtn setTitleColor:[JKUtil getColor:@"108ee9"] forState:UIControlStateHighlighted];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    [[PhotoAlbumManager sharedManager] saveImage:image
                                 ...
                                 completionBlock:^(ALAsset *asset, NSError *error) {
                                     if (error == nil && asset) {
                                         NSURL *assetURL = [asset valueForProperty:ALAssetPropertyAssetURL];
                                         [self addAssetsObject:assetURL Img:image];
...
- (void)didSelectItemAssetsViewCell:(JKAssetsViewCell *)assetsCell
{
    ...
    if (validate) {
        ...
        UIImage *img =  [UIImage imageWithCGImage:[[assetsCell.asset defaultRepresentation] fullScreenImage]];
...
- (void)addAssetsObject:(NSURL *)assetURL realImg:(UIImage *)img
{
    ...
    asset.photo = img;
//    ALAssetsLibrary *lib = _assetsLibrary;
//    
...
- (void)addAssetsObject:(NSURL *)assetURL Img:(UIImage *)img
{
    NSURL *groupURL = [self.selectAssetsGroup valueForProperty:ALAssetsGroupPropertyURL];
    NSString *groupID = [self.selectAssetsGroup valueForProperty:ALAssetsGroupPropertyPersistentID];
    JKAssets  *asset = [[JKAssets alloc] init];
...
- (UILabel *)titleLabel{
    if (!_titleLabel) {
        ...
        _titleLabel.textColor = FirstColor;
...
- (UIView *)toolbar{
    if (!_toolbar) {
        ...
        _toolbar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.height-44-kTopHeight - bottom, self.view.width, 44)];
...
//        [_selectButton addSubview:label];
        ...
        _finishLabel.text = @"下一步(6)";
        [_finishLabel sizeToFit];
        
        _finishButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _finishButton.frame = CGRectMake(0, 5, _finishLabel.width+10, 34);
...
//        [_finishButton setBackgroundImage:[JKUtil stretchImage:[UIImage imageNamed:@"picker_button_orange_highlighted"] capInsets:UIEdgeInsetsMake(5, 5, 5, 5) resizingMode:UIImageResizingModeStretch] forState:UIControlStateHighlighted];
        _finishButton.backgroundColor = MainBg_Color;
...
- (UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, self.view.width, [UIScreen mainScreen].bounds.size.height-kTopHeight-44 - bottom) collectionViewLayout:layout];
