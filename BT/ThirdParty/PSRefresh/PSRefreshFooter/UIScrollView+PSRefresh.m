...
- (void)addRefreshHeaderWithClosure:(PSRefreshClosure)closure {
    if (!self.header && !self.gifHeader) {
        ...
        self.header.stateLabelHidden = NO;
        [self.header setTitle:@"右拉即可加载之前数据" forState:PSRefreshStatePullCanRefresh];
        [self.header setTitle:@"松开即可加载" forState:PSRefreshStateReleaseCanRefresh];
        [self.header setTitle:@"正在为您加载" forState:PSRefreshStateRefreshing];
...
//        for (NSUInteger i = 1; i <= 60; i++) {
            UIImage *image = [UIImage imageNamed:@"Loadingdown_0.png"];
...
//        }
        ...
        for (NSUInteger i = 0; i < 49; i++) {
            UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"Loadingdown_%zd.png", i]];
...
//        for (NSUInteger i = 1; i <= 60; i++) {
            UIImage *image = [UIImage imageNamed:@"loading_0"];
